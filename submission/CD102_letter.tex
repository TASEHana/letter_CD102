...
$\left|g_{a\gamma\gamma}\right|\gtrsim 8.2\times 10^{-14}\,\text{Ge\hspace{-.08em}V}^{-1}$, a factor of eleven above the benchmark KSVZ model, reaching 
...
constraints on $g_{a\gamma\gamma}$ in this mass region.
...
Axion Search Experiment with Haloscope (TASEH). 
...
diagram of the TASEH apparatus. An 8-Tesla superconducting solenoid with a 
...
cooling: 50~K, 4~K, still, and mixing flanges, among which the mixing flange 
could reach the lowest temperature at $\simeq20$~mK. During the data taking, 
the MW cavity with two coupling probes sits in the center of the magnet bore 
and is connected via holders to the mixing flange. The $0.234$-L cylindrical 
...
copper tuning rod. The axion-photon conversion signal from the readout probe 
...
respectively. The form factor $C$ for the TM$_{010}$ mode varies from 0.60 to 
0.61 over the operational frequency range. The intrinsic quality factor $Q_0$ 
...
about $1.9-2.2$~K, with a frequency dependence. In CD102, 
...
range from $5.3\times 10^{-14}\,\text{Ge\hspace{-.08em}V}^{-1}$ to 
$8.9\times 10^{-14}\,\text{Ge\hspace{-.08em}V}^{-1}$, with an average 
...
\begin{figure*} 
  \centering
  \includegraphics[width=12.9cm]{Figure2.png}
 ...
 performed by the ADMX, CAPP, and HAYSTAC Collaborations are also shown 
...
the axion-two-photon coupling $\left|g_{a\gamma\gamma}\right|\gtrsim 8.2\times 10^{-14}\,\text{Ge\hspace{-.08em}V}^{-1}$ at 95\% C.L.,
...
better than the existing limits in the same mass range.  
It is also the first time that a haloscope-type experiment places 
constraints in this mass region. The target of TASEH is to search for axions 
